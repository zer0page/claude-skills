...
2. If `git config user.name` is empty, copy identity from the parent repo: `git log -1 --format='%an'` and `git log -1 --format='%ae'`, then set with `git config user.name` and `git config user.email`
3. Enter plan mode
...
3. On approval:
   ...
   - Delete the remote branch: `BRANCH=$(gh pr view --json headRefName --jq '.headRefName') && git push origin --delete "$BRANCH"`
